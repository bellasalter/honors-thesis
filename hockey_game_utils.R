...
get_all_info <- function(year, all_game_ids) {
  ...
  for(game in game_ids) {
    ...
    for(player in player_info$playerId) {
      pos <- complete_roster[complete_roster$year == curr_yr & complete_roster$id == player,]
    ...
    for(i in 1:nrow(play_data)) {
      ...
      if(row$type == "faceoff") {
        ...
        tmAbbrev <- complete_roster[complete_roster$year == curr_yr & complete_roster$id == prim_player,]$team
        #print(tmAbbrev)
  ...
  write.csv(big_player_df, sprintf("/%s_df/big_player_df.csv", year))
  write.csv(big_team_df, sprintf("/%s_df/big_team_df.csv", year))
  write.csv(giant_df, sprintf("/%s_df/giant_df.csv", year))
}
...
compile_team_abbrevs <- function(pbp, tm1, tm2, yr) {
  ...
  for(r in c(1:nrow(pbp))) {
    ...
    player_tm <- "000"
    if(player != "0") {
      poss_tms <- complete_roster[complete_roster$year == yr & complete_roster$id == player,]$team
...
process_shot_stats <- function(play_by_play_df, stats_df, Szn) {
  ...
  if(nrow(stats_df) > 0) {
    for(i in 1:nrow(stats_df)) {
      player <- stats_df[i,]
      results[[as.character(player$PlayerId)]] <- list(
        PlayerId = player$PlayerId,
        Szn = player$Szn,
        tmAbbrev = player$tmAbbrev,
        Pos = player$Pos,
  ...
  for (i in 1:nrow(play_by_play_df)) {
    ...
    if (!is.na(row$playerIDs) && row$playerIDs != "") {
      ...
      if (!shooter_id %in% names(results)) {
        roster_info <- complete_roster[complete_roster$year == Szn & complete_roster$id == shooter_id,]
